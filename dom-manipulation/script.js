...
function syncQuotes(serverQuotes) {
  ...
  localQuotes.forEach(localQuote => {
    ...
    if (serverQuote) {
      ...
      // If no server quote matches, add local quote to the server's collection
  ...
  if (conflicts.length > 0) {
    conflicts.forEach(conflict => {
      ...
      if (index !== -1) {
        ...
        notifyUser('Quote updated from server.'); // Notify user about conflict resolution
      }
    });
  } else {
...
function notifyUser(message) {
  alert(message);
  // Optional: You can also implement a more sophisticated UI notification system
}
...
// Other existing functions (populateCategories, filterQuotes, etc.) go here...